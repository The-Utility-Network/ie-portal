  ...
  return (
    <div
      ...
      style={{
        ...
        borderColor: isRead ? 'rgba(127, 44, 255, 0.3)' : 'rgba(147, 51, 234, 0.3)', // Purple border
        boxShadow: executing ? `0 0 30px ${isRead ? '#7f2cff' : '#9333ea'}` : '0 10px 30px rgba(0,0,0,0.5)',
      ...
      <div
        ...
        style={{
          backgroundImage: isRead
            ? 'linear-gradient(90deg, rgba(127, 44, 255, 0.15), transparent)'
            : 'linear-gradient(90deg, rgba(147, 51, 234, 0.15), transparent)'
        }}
      >
        <div
        ...
        >
          {isRead ? <PlayCircleIcon className="w-5 h-5 text-[#7f2cff]" /> : <BoltIcon className="w-5 h-5 text-purple-400" />}
      ...
      <div className="p-4 space-y-4 relative">
            ...
            {inputs.map((input, idx) => (
              <div key={idx} className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  style={{
                    borderColor: isRead ? 'rgba(127, 44, 255, 0.2)' : 'rgba(147, 51, 234, 0.2)'
                  }}
                  onFocus={(e) => e.target.style.borderColor = isRead ? '#7f2cff' : '#9333ea'}
                  onBlur={(e) => e.target.style.borderColor = isRead ? 'rgba(127, 44, 255, 0.2)' : 'rgba(147, 51, 234, 0.2)'}
                  placeholder="Value..."
                  value={input.value}
        ...
        <div className="pt-2 flex gap-2">
          <button
            ...
            style={{
              background: isRead ? 'rgba(127, 44, 255, 0.15)' : 'rgba(147, 51, 234, 0.15)',
              border: `1px solid ${isRead ? 'rgba(127, 44, 255, 0.4)' : 'rgba(147, 51, 234, 0.4)'}`,
              color: isRead ? '#7f2cff' : '#a855f7',
            }}
          >
      ...
      {outputModalOpen && (
        <div className="absolute inset-0 bg-black/95 z-[60] flex flex-col p-4 animate-in fade-in zoom-in duration-200">
          <div className="flex justify-between items-center mb-2 pb-2 border-b border-white/10">
            <span className="text-xs font-mono text-[#7f2cff] uppercase tracking-widest">Execution Result</span>
...
const Workspace: React.FC<WorkspaceProps> = ({ allABIs, diamondAddress }) => {
  ...
  return (
    <div
      ...
      onWheel={handleWheel}
    >
      {/* Tech Grid Background (Moving) - Purple Version */}
      <div
        ...
        style={{
          ...
          transformOrigin: '0 0',
          backgroundImage: `
              linear-gradient(rgba(127, 44, 255, 0.05) 1px, transparent 1px),
              linear-gradient(90deg, rgba(127, 44, 255, 0.05) 1px, transparent 1px)
            `,
      ...
      <div className="absolute top-4 right-4 z-[100] flex gap-2">
        <div className="flex bg-black/50 backdrop-blur-md border border-white/10 rounded-lg p-1">
          <button onClick={() => setScale(s => Math.min(s * 1.2, 3))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-[#7f2cff]"><MagnifyingGlassPlusIcon className="w-5 h-5" /></button>
          <button onClick={() => setScale(s => Math.max(s / 1.2, 0.2))} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-[#7f2cff]"><MagnifyingGlassMinusIcon className="w-5 h-5" /></button>
          <div className="w-[1px] bg-white/10 mx-1" />
          <button onClick={() => { setScale(1); setPosition({ x: 0, y: 0 }); }} className="p-2 hover:bg-white/10 rounded text-white/70 hover:text-[#7f2cff]"><ArrowPathIcon className="w-5 h-5" /></button>
      ...
      {panels.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
          <div className="flex flex-col items-center">
            <CubeTransparentIcon className="w-24 h-24 text-[#7f2cff] mb-4" />
      ...
      {clearModalOpen && (
        <div className="absolute inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-[#050a14] border border-[#7f2cff]/30 rounded-lg p-6 max-w-sm w-full shadow-[0_0_30px_rgba(127,44,255,0.2)]">
            <h3 className="text-[#7f2cff] font-mono font-bold text-lg mb-2 tracking-widest">CLEAR WORKSPACE</h3>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded bg-[#7f2cff]/10 border border-[#7f2cff]/50 text-[#7f2cff] hover:bg-[#7f2cff]/20 text-[10px] font-bold tracking-wider transition-all shadow-[0_0_20px_rgba(127,44,255,0.1)] hover:shadow-[0_0_30px_rgba(127,44,255,0.3)]"
              >
                CONFIRM_CLEAR
              </button>
            </div>
...
const MethodItem = ({ methodName, isRead, inputs, outputs }: any) => {
  ...
  return (
    <div ref={drag as unknown as React.LegacyRef<HTMLDivElement>} className={`group p-3 rounded-md border cursor-grab transition-all duration-200 relative overflow-hidden ${isDragging ? 'opacity-50' : 'opacity-100 hover:scale-[1.02]'
        ...
        ? 'bg-[#7f2cff]/10 border-[#7f2cff]/20 hover:border-[#7f2cff]/60'
        : 'bg-purple-900/20 border-purple-500/20 hover:border-purple-400/60'
      }`}>
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${isRead ? 'bg-[#7f2cff]' : 'bg-purple-500'} opacity-50`} />
      <div className="flex justify-between items-center pl-2">
        <span className={`font-mono text-[11px] font-bold truncate ${isRead ? 'text-[#7f2cff]' : 'text-purple-300'}`}>
          {methodName}
        </span>
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setDiamondAddress(addr);
        console.log("Directory: Resolved Diamond Address:", addr);
        ...
        facetAddresses.forEach(addr => {
          ...
          if (fetched) {
            console.log(`Facet at ${addr}: ${fetched.name}`); // Debug log
            const targetName = process.env.NEXT_PUBLIC_DIRECTORY_FACET || 'InvisibleEnemiesDirectoryv1';
            console.log(`Target Facet Name: ${targetName}`);

            // Flexible matching: use env var or default (case-insensitive)
            if (fetched.name && fetched.name.toLowerCase().includes(targetName.toLowerCase())) {
              console.log("MATCH FOUND for", fetched.name);
    ...
    <DndProvider backend={HTML5Backend}>
      <div
        ...
        style={{
          ...
          border: '1px solid rgba(127, 44, 255, 0.2)', // Purple Border
          boxShadow: '0 0 40px rgba(127, 44, 255, 0.1)', // Purple Glow
        }}
      >
        ...
        {/* Sidebar / Bottom Bar */}
        <div className="w-full md:w-72 h-32 md:h-full flex-shrink-0 flex flex-col border-t md:border-t-0 md:border-r border-[#7f2cff]/20 bg-[#050a14]/90 backdrop-blur-xl z-20 order-2 md:order-1">
          <div className="p-3 md:p-5 border-b border-[#7f2cff]/20 bg-black/20 flex justify-between md:block items-center">
            <div>
              <h2 className="font-bold text-sm md:text-lg text-white tracking-[0.2em] mb-1">DIRECTORY</h2>
              <div className="text-[9px] md:text-[10px] text-[#7f2cff] font-mono tracking-widest flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-[#7f2cff] animate-pulse" />
                SYSTEM.ACTIVE
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-x-auto md:overflow-y-auto overflow-y-hidden md:overflow-x-hidden p-2 md:p-3 space-x-4 md:space-x-0 md:space-y-6 custom-scrollbar flex flex-row md:flex-col items-center md:items-stretch">
            {loading ? (
              <div className="text-center py-4 md:py-20 space-x-4 md:space-x-0 md:space-y-4 flex md:block items-center justify-center w-full">
                <div className="w-6 h-6 md:w-10 md:h-10 border-2 border-[#7f2cff] border-t-transparent rounded-full animate-spin mx-auto" />
                <div className="text-[10px] font-mono text-[#7f2cff] animate-pulse tracking-widest whitespace-nowrap">
              ...
              <>
                ...
                <div className="flex-shrink-0 flex flex-row md:flex-col gap-2 md:gap-0 items-center md:items-stretch h-full md:h-auto">
                  <h3 className="hidden md:block text-[10px] font-bold text-white/40 mb-3 uppercase tracking-[0.15em] pl-1 border-b border-white/5 pb-1">Write / Actions</h3>
                  <div className="flex md:flex-col gap-2">
                    {methods.write.map((m) => {
                      const details = getMethodDetails(m, allABIs);
                      return (
